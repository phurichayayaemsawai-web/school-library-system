...
import { Bookmark, BookOpen, Clock, TrendingUp, BookCheck, Plus } from "lucide-react";
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <section className={`grid grid-cols-1 ${isAdmin ? "sm:grid-cols-2 lg:grid-cols-4" : "sm:grid-cols-3"} gap-3 sm:gap-4`}>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-sky-100 shadow-sm hover:border-sky-200 transition-all flex items-center justify-between">
          <div className="min-w-0">
            <p className="text-xs font-medium text-slate-500 truncate whitespace-nowrap">
              หนังสือในคลัง
            </p>
            <h3 className="text-xl sm:text-2xl font-black text-blue-600 mt-1 truncate">
              {books.length} เล่ม
            </h3>
            <p className="text-[11px] text-slate-400 mt-0.5 truncate whitespace-nowrap">
              พร้อมให้ยืม {availableBooks.length} เล่ม
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-sky-100 shadow-sm hover:border-sky-200 transition-all flex items-center justify-between">
          <div className="min-w-0">
            ...
            <h3 className="text-xl sm:text-2xl font-black text-amber-600 mt-1 truncate">
              {borrowedBooks.length} เล่ม
            </h3>
            <p className="text-[11px] text-slate-400 mt-0.5 truncate whitespace-nowrap">
              เกินกำหนด {overdueTrx.length} เล่ม
            </p>
          </div>
        ...
        {isAdmin && (
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-sky-100 shadow-sm hover:border-sky-200 transition-all flex items-center justify-between animate-in fade-in">
            <div className="min-w-0">
              ...
              <h3 className="text-xl sm:text-2xl font-black text-emerald-600 mt-1 truncate">
                {validTrx.length} ครั้ง
              </h3>
              <p className="text-[11px] text-slate-400 mt-0.5 truncate whitespace-nowrap">
                คืนสำเร็จ {returnedTrx.length} ครั้ง
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
            {previewBooks.map((book) => (
              <BookCard
                key={book.id}
                book={book}
                onView={(b) => setSelectedViewBook(b)}
      ...
      <BookModal
        ...
        onClose={() => setSelectedViewBook(null)}
        onBorrow={(b) => setSelectedBorrowBook(b)}
      />
