...
import { ShieldAlert, ArrowLeft, BookOpen, CircleCheck, Image as ImageIcon, Upload, Scan, Hash, Calendar, MapPin, Bookmark } from "lucide-react";
...
export default function NewBookPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addBook({
      ...
      author: author.trim(),
      category,
      isbn: isbn.trim() || `978-616-${Math.floor(1000 + 9000 * Math.random())}-${Math.floor(10 + 90 * Math.random())}`,
      ...
      {isSuccess ? (
        <div className="bg-white rounded-2xl sm:rounded-3xl p-10 sm:p-12 text-center border border-sky-100 shadow-sm space-y-3">
          ...
          <h2 className="text-base sm:text-lg font-bold text-slate-800">
            ลงทะเบียนหนังสือรหัส "{bookId}" เรียบร้อยแล้ว!
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <img
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      "https://placehold.co/400x600/e2e8f0/475569?text=Cover+Preview";
                  }}
                />
              </div>
          ...
          <div className="md:col-span-2">
            <form
              ...
              className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-sky-100 shadow-sm space-y-5"
            >
