...
import { Book, BorrowerType, STUDENT_GRADES, STUDENT_ROOMS, SCHOOL_DEPARTMENTS } from '@/types';
...
export const TeacherBorrowDesk: React.FC<TeacherBorrowDeskProps> = ({ onSuccess }) => {
  ...
  const [studentGrade, setStudentGrade] = useState<string>(STUDENT_GRADES[0]); // มัธยมศึกษาปีที่ 1
  const [studentRoom, setStudentRoom] = useState<string>(STUDENT_ROOMS[0]); // ห้อง 1
  ...
  // Live lookup book whenever bookIdInput changes (supports ID, ISBN, Title, Author)
  useEffect(() => {
    setError(null);
    ...
    const found =
      books.find((b) => b.id.toLowerCase() === query || b.isbn.toLowerCase() === query) ||
      books.find(
        (b) =>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (borrowerType === 'STUDENT') {
      if (!studentName.trim() || !studentId.trim() || !studentGrade || !studentRoom) {
        setError('กรุณากรอกชื่อ-นามสกุล, เลขประจำตัว, ระดับชั้น และห้องเรียน ให้ครบถ้วน');
        return;
      }
      borrowerData = {
      ...
      <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
        ...
        <div className="p-4 sm:p-5 bg-sky-50/50 border border-sky-100 rounded-2xl sm:rounded-3xl space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                required
                placeholder="เช่น TH-001, วรรณคดี, 974-01-2602-2, ......"
                value={bookIdInput}
          ...
          {matchedBook ? (
            ...
            <p className="text-xs text-sky-600 italic">กำลังค้นหาข้อมูลหนังสือ "{bookIdInput}" ในระบบ...</p>
          ) : (
        ...
        <div className="space-y-3.5">
          ...
          {/* Student Form */}
          {borrowerType === 'STUDENT' ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 p-4 bg-sky-50/30 rounded-2xl border border-sky-100">
              <div>
                <label className="block text-[11px] font-semibold text-slate-600 mb-1 whitespace-nowrap">
                  เลขประจำตัวนักเรียน <span className="text-blue-600 font-bold">*</span>
                </label>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-600 mb-1 whitespace-nowrap">
                  ชื่อ-นามสกุล นักเรียน <span className="text-blue-600 font-bold">*</span>
                </label>
                <input
                  type="text"
                  required
                  placeholder="เช่น ด.ช. กิตติศักดิ์ ใจดี"
                  value={studentName}
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-600 mb-1 whitespace-nowrap">
                  ระดับชั้น <span className="text-blue-600 font-bold">*</span>
                </label>
                <select
                  required
                  value={studentGrade}
                  onChange={(e) => setStudentGrade(e.target.value)}
                  className="w-full px-2 py-2 bg-white border border-sky-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-blue-400 focus:outline-none cursor-pointer"
                >
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-600 mb-1 whitespace-nowrap">
                  ห้อง <span className="text-blue-600 font-bold">*</span>
                </label>
                <select
                  required
                  value={studentRoom}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white border border-sky-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>
            </div>
          ) : (
