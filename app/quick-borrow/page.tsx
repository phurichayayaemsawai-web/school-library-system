...
import { STUDENT_GRADES, STUDENT_ROOMS, SCHOOL_DEPARTMENTS, BorrowerType } from '@/types';
...
export default function QuickBorrowPage() {
  ...
  const [grade, setGrade] = useState<string>(STUDENT_GRADES[0]); // มัธยมศึกษาปีที่ 1
  const [room, setRoom] = useState<string>(STUDENT_ROOMS[0]); // ห้อง 1
  ...
  const handleBorrowSubmit = (e: React.FormEvent) => {
    ...
    if (borrowerType === 'STUDENT') {
      if (!studentId.trim() || !studentName.trim() || !grade || !room) {
        showToast('กรุณากรอกเลขประจำตัว, ชื่อ-นามสกุล, ระดับชั้น และห้องเรียนให้ครบถ้วน', 'error');
        return;
      }
    } else {
      if (!teacherName.trim()) {
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1 whitespace-nowrap">
                    ระดับชั้น <span className="text-blue-600">*</span>
                  </label>
                  <select
                    required
                    value={grade}
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1 whitespace-nowrap">
                    ห้องเรียน <span className="text-blue-600">*</span>
                  </label>
                  <select
                    required
                    value={room}
