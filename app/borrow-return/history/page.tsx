...
export default function HistoryPage() {
  ...
  const returnedCount = validTransactions.filter((t) => t.status === 'RETURNED').length;
  const activeCount = validTransactions.filter((t) => t.status === 'ACTIVE' || t.status === 'OVERDUE').length;
  ...
  const handleExportCSV = () => {
    // Generate simple CSV from validTransactions
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-white rounded-xl sm:rounded-2xl p-4 border border-sky-100 shadow-2xs">
          ...
          <p className="text-xl sm:text-2xl font-black text-slate-800 mt-1 truncate">{validTransactions.length} รายการ</p>
        </div>
        <div className="bg-white rounded-xl sm:rounded-2xl p-4 border border-sky-100 shadow-2xs">
